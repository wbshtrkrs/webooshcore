<div>
    <input class="form-control control-half autonumeric" value="{{ $model->getValue($key, $listItem, $language) }}" type="text" {{ $model->isRequired($key) }} {{ $model->isDisabled($key) }} label="{{ $model->label($key) }}" placeholder="{{ $model->placeholder($key) }}"/>
